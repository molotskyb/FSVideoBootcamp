...
const keyGroupId = readOutput("ffout_cf_keygroup_id.txt");
const fetchedConfigPath = getAwsOutputFilePath("ffout_cf_restrict_on_fetched.json");
const updateConfigPath = getAwsOutputFilePath("ffout_cf_restrict_on_update.json");
const updateResponsePath = getAwsOutputFilePath("ffout_cf_restrict_on_result.json");
...
const updateText = run(
	`aws cloudfront update-distribution --id ${distributionId} --if-match ${etag} --distribution-config file://${getAwsOutputProjectRelativePath("ffout_cf_restrict_on_update.json")}`,
	{ capture: true },
);
...
console.log(
	`Updated distribution ${distributionId}; DefaultCacheBehavior now requires trusted key group ${keyGroupId}.`,
);
